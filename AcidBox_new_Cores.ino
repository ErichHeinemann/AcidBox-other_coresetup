/*
  TEST !!!
  E.Heinemann, one Core is doing Audio, the other Core is doing UI, GPIOs etc...

  TESTS:
  Control Instrument-Mute via MIDI-Keyboard ... all keys < 48
  MIDI_Clock, Start/Stop 
  8 Pots via Analog Multiplexer  

  Identified Problems:
  Not enough Power to switch Multiplexer Adresses and do Ovesampßling of ADC >> sounds acceptable with lower Samplee-Rate
  Tests with 4 Pots ok
  Buttons lacking, Start/Stop to late  
...
// E.Heinemann - Variables for the UI
...
/*    
    if (timer2_fired) {
      timer2_fired = false;
      // DEB ("XXXX");
      // E.Heinemann, Multipßplexer for Pots ..
      AdcMul_Process();
...
void setup(void) {
  ...
  */
  ...
  // E.Heinemann, enable Pots using Multiplexer
  // This is always the choice used by Marcel Licence, I prefer more ADC1115 but they create problems too... 
  // AdcMul_Init();
  ...
  xTaskCreatePinnedToCore( Core0Task, "Core0Task", 8000, NULL, 0, &Core0TaskHnd, 0);
  ...
  timerAlarmWrite(timer2, 200000, true);          // 200ms, autoreload 200000
...
/*
  ...
  // put your loop stuff for core0 here
   if( prescaler==3 ){
    readPcf();  // PCF8574 for the 16 Steps
   }
  ...
  if( display_updated==false && prescalerDisplay==5){
    updateLeds();    
  }

  if( display_updated==false && prescalerDisplay > 11 ){
    updateDisplay();  
    prescalerDisplay=0;
    display_updated=true;
  }

  if( prescalerDisplay > 12 ){
    prescalerDisplay=0;
  }
  */
}
...
void Core0TaskSetup(){
  ...
  // I2Ctwo.begin( SDA2, SCL2, 100000 ); // SDA2 pin 0, SCL2 pin 5, works with 50.000 Hz if pinned to other Core0
  // Input, MPR121, PCF8574, ADS1115
  //delay(10);
  //setupTouch();
  delay(10);
  //setupPcf();
...
void Core0Task( void *parameter ){
  ...
  while( true ){
    Core0TaskLoop();
    delay(1); // this seems necessary to trigger the watchdog
    yield();
  }
}
...
void xxloop() { // default loopTask running on the Core1
